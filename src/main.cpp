...
// include directory
#include "deneme/FusionEKF.h"
#include "deneme/measurement_package.h"
...
cv::KalmanFilter KF8(stateDim, measDim, ctrlDim, CV_32F);
...
void KFT(const std_msgs::Float32MultiArray ccs) {
  ...
  std::vector<cv::Mat> pred{KF0.predict(), KF1.predict(), KF2.predict(),
                            ...
                            KF6.predict(), KF7.predict(), KF8.predict()};
  ...
  // Get measurements
  // Extract the position of the clusters forom the multiArray. 
  ...
  objID.resize(9); // Allocate default elements so that [i] doesnt segfault.
  ...
  for (int filterN = 0; filterN < 9; filterN++) {
  ...
  for (int clusterCount = 0; clusterCount < 9; clusterCount++) {
    ...
    distMat[minIndex.first] =
        std::vector<float>(9, 10000.0); // Set the row to a high number.
  ...
  for (int i = 0; i < 9; i++) {
    ...
    m.color.a = 1.0;
    m.color.r = i % 2 ? 1 : 0; //for different colors
    m.color.g = i % 3 ? 1 : 0;
    m.color.b = i % 4 ? 1 : 0;
  ...
  float meas6[2] = {cc[6].at(0), cc[5].at(1)};
  float meas7[2] = {cc[7].at(0), cc[5].at(1)};
  float meas8[2] = {cc[8].at(0), cc[5].at(1)};
  ...
  cv::Mat meas8Mat = cv::Mat(2, 1, CV_32F, meas8);
  ...
  if (!(meas8[0] == 0.0f || meas8[1] == 0.0f))
    Mat estimated8 = KF8.correct(meas8Mat);
...
// callbakc for subsribe function
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr &input){

  // ros::Duration(1).sleep(); //frequenct is 1 second
  ...
  // objects
  if (firstFrame) {
    // Initialize 9 Kalman Filters; Assuming 9 max objects in the dataset.
    ...
    KF5.transitionMatrix = (Mat_<float>(4, 4) << dx, 0, 1, 0, 0, dy, 0, 1, 0, 0,
                            dvx, 0, 0, 0, 0, dvy);
    KF6.transitionMatrix = (Mat_<float>(4, 4) << dx, 0, 1, 0, 0, dy, 0, 1, 0, 0,
                            dvx, 0, 0, 0, 0, dvy);
    KF7.transitionMatrix = (Mat_<float>(4, 4) << dx, 0, 1, 0, 0, dy, 0, 1, 0, 0,
                            dvx, 0, 0, 0, 0, dvy);
    KF8.transitionMatrix = (Mat_<float>(4, 4) << dx, 0, 1, 0, 0, dy, 0, 1, 0, 0,
                            dvx, 0, 0, 0, 0, dvy);

    // getting referance datas through opencv Identity function
    ...
    setIdentity(KF8.processNoiseCov, Scalar::all(sigmaP));
    ...
    // converting rosmsg to pcl data 1.0, - 5
    pcl::fromROSMsg(*input, *input_cloud);

    tree->setInputCloud(input_cloud);

    std::vector<pcl::PointIndices> cluster_indices;
    pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec; //for data assosiaction

    ec.setClusterTolerance(1.2); // default is 8 cm
    ec.setMinClusterSize(1); //default is 10
    ec.setMaxClusterSize(2500);
    ec.setSearchMethod(tree);
    ec.setInputCloud(input_cloud);

    cout<<"about to excrating clusters \n";
    
    // in case these is no indices, pass the exctraction 
    ec.extract(cluster_indices); /* Extract the clusters out of pc and save indices in cluster_indices.*/

    cout<<"excracting process is finished \n";
    ...
    // Ensure at least 9 clusters exist to publish (later clusters may be empty)
    while (cluster_vec.size() < 9) {
    ...
    // Set initial state
    KF8.statePre.at<float>(0) = clusterCentroids.at(8).x;
    KF8.statePre.at<float>(1) = clusterCentroids.at(8).y;
    ...
    try{
      // filter for NAN type messages
      // std::shared_ptr<std::vector<int>> indicess(new std::vector<int>);
      // pcl::removeNaNFromPointCloud(*input_cloud, *input_cloud2, *indicess);
      // pcl::ExtractIndices<pcl::PointXYZ> extract;
      // extract.setInputCloud(input_cloud2);
      // // extract.setIndices(indices);
      // extract.setNegative(true);
      // extract.filter(*input_cloud2);

      tree->setInputCloud(input_cloud);

      /* Here is creating a vector of PointIndices, which contains the actual
      ...
      * indices of the first cluster in input point cloud.*/
      ...
      //for euclideanCluster parameters, visit this site: https://pcl.readthedocs.io/en/latest/cluster_extraction.html
      ...
      // in case these is no indices, pass the exctraction as a empty matrix
      try{
      ...
      KFT(cc);
      int i = 0;
      bool publishedCluster[9];
      cout<<"9 clusters initialized\n";
...
int main(int argc, char **argv) {
  ...
  // waiting for ros service
  ros::Duration(3).sleep(); // sleep for one second to overcome first datas
  ...
  cout<<"listening at the filtered_coud topic"<<endl;
